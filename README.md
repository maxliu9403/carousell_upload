...
启动后将新增“运行模式”交互，默认有人值守：

- 1. 有人值守（默认）：遇到需要更新 CSS 选择器时，会交互提示输入；
- 2. 无人值守：遇到需要更新 CSS 选择器时，自动采用“当前主选择器”（若不存在主选择器，则自动跳过当前商品）。
...
- 仅使用主选择器：已移除 fallback 选择器逻辑，配置与运行更简单稳定。
- 显示当前选择器：当需要更新时，会在终端显示“当前主选择器”，方便复制参考。
- 快捷键：
  - 输入 `d`：直接使用当前主选择器；
  - 输入 `q`：退出程序；
  - 输入 `next`：退出当前流程，继续下一个商品；
  - 对于非必须元素，输入 `skip`：跳过该步骤。
- 防呆处理：
  - 无人值守模式下，自动使用当前主选择器；若无主选择器，则自动跳过该商品；
  - 当“使用新选择器操作仍失败”时，自动跳过当前商品，继续下一个。
...
- 新增运行模式选择：支持“有人值守 / 无人值守”（默认有人值守）。
- 选择器交互优化：支持输入 `d` 直接使用当前主选择器；失败后自动跳过当前商品。
- 仅主选择器：移除所有 fallback 逻辑与配置；运行时仅使用 `primary`。
- IP 地域校验：启动浏览器后自动校验当前 IP 所在地域是否与期望地域一致（不一致则关闭并跳过）。
- IxBrowser 启动重试：当接口返回 `code=1003` 时自动重试 3 次，每次间隔 10 秒。
